...
ColumnLayout {
    ...
    function checkPopout(y: real): void {
        ...
        if (id === "statusIcons" && Config.bar.popouts.statusIcons) {
            ...
            popouts.currentName = "power";
            popouts.currentCenter = (ch.item as Item).mapToItem(root, 0, (ch.item as Item).implicitHeight / 2).y ?? 0;
            popouts.hasCurrent = true;
        }
    }

    function togglePopoutAt(y: real): void {
        const previousName = popouts.currentName;
        const hadCurrent = popouts.hasCurrent;

        checkPopout(y);

        if (hadCurrent && popouts.hasCurrent && popouts.currentName === previousName)
            popouts.hasCurrent = false;
    }
