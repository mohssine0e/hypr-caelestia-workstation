...
QtObject {
    ...
    readonly property string displayBody: cleanBody(body)
    ...
    function stripMarkup(text: string): string {
        return decodeEntities(text.replace(new RegExp("<a\\b[^>]*>(.*?)</a>", "gi"), "$1").replace(new RegExp("<[^>]+>", "g"), ""));
    }

    function isSourceLine(text: string): bool {
        const clean = text.trim();
        return new RegExp("^(https?://)?([a-z0-9-]+\\.)+[a-z]{2,}(/.*)?$", "i").test(clean);
    }

    function cleanBody(text: string): string {
        const raw = String(text ?? "").replace(new RegExp("\\r", "g"), "").replace(new RegExp("<br\\s*/?>", "gi"), "\n");
        const lines = raw.split(new RegExp("\\n+")).map(line => stripMarkup(line).trim()).filter(line => line.length > 0);
        if (lines.length > 1 && isSourceLine(lines[0]))
