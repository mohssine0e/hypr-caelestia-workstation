...
Singleton {
    ...
    readonly property HyprKeyboard keyboard: extras.devices.keyboards.find(kb => kb.main) ?? extras.devices.keyboards.find(kb => String(kb.name ?? "").includes("keyboard")) ?? extras.devices.keyboards[0] ?? null
    readonly property bool capsLock: keyboard?.capsLock ?? false
    readonly property bool numLock: keyboard?.numLock ?? false
    readonly property string defaultKbLayout: layoutCodeFromToken(keyboard?.layout?.split(",")[0] ?? "")
    readonly property string kbLayoutFull: keyboard?.activeKeymap ?? ""
    readonly property string kbLayout: layoutCodeFromFull(kbLayoutFull)
    ...
    function layoutCodeFromToken(token: string): string {
    ...
    function layoutCodeFromFull(name: string): string {
        const full = String(name ?? "").trim();
        if (!full || full === "Unknown")
            return defaultKbLayout || "??";

        const mapped = kbMap.get(full);
        if (mapped)
            return layoutCodeFromToken(mapped);

        if (full.includes("Arabic"))
            return "AR";
        if (full.includes("French"))
            return "FR";
        if (full.includes("English") || full.includes("US"))
            return "US";
