...
Singleton {
    ...
    property real diskTemp
    property real batteryTemp
    ...
    // Network counters are bytes since boot, rates are bytes/sec.
    property real netRxBytes
    property real netTxBytes
    property real netDownRate
    property real netUpRate
    property real lastNetRxBytes
    property real lastNetTxBytes
    property real lastNetMs
    ...
    FileView {
        ...
        printErrors: false
        onLoaded: root.cpuFreqMhz = (parseInt(text().trim(), 10) || 0) / 1000
    }
    ...
    FileView {
        ...
        onLoaded: {
            ...
            for (const rawLine of text().split("\n")) {
                ...
                const parts = dataRaw.trim().split(/\s+/).map(n => parseInt(n, 10) || 0);
                if (parts.length < 16)
                    continue;
            ...
            const now = Date.now();
            if (root.lastNetMs > 0) {
                const seconds = Math.max(0.001, (now - root.lastNetMs) / 1000);
                root.netDownRate = Math.max(0, (rx - root.lastNetRxBytes) / seconds);
                root.netUpRate = Math.max(0, (tx - root.lastNetTxBytes) / seconds);
            }

            root.netRxBytes = rx;
            root.netTxBytes = tx;
            root.lastNetRxBytes = rx;
            root.lastNetTxBytes = tx;
            root.lastNetMs = now;
        }
    }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                let diskTemp = text.match(/(?:Composite|Sensor 1):\s+\+([0-9.]+)(°| )C/);
                if (!diskTemp)
                    diskTemp = text.match(/temp1:\s+\+([0-9.]+)(°| )C/);
                if (diskTemp)
                    root.diskTemp = parseFloat(diskTemp[1]);

                const batBlock = text.match(/BAT[0-9]+-[\\s\\S]*?(?=\\n\\S|$)/);
                const batTemp = batBlock ? batBlock[0].match(/temp[0-9]*:\\s+\\+([0-9.]+)(°| )C/) : null;
                root.batteryTemp = batTemp ? parseFloat(batTemp[1]) : 0;
